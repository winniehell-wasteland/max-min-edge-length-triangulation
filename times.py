...
def output_time_comparison(data, instances):
    ...
    results.output('time_comparison', data=data, columns=columns, aggregations=aggregations, instances=instances)
...
def output_time_total(data, instances):
    ...
    results.output('time_total', data=data, columns=columns, aggregations=aggregations, instances=instances)
...
def output_time_noncrossing(data, instances, threshold):
    ...
    data = data[columns.keys()]
    data = data.agg(aggregations)
    data = data[data.index <= np.min([i for i in data.index if instances[i] < threshold])]
    ...
    results.output('time_noncrossing', data=data, columns=columns, aggregations=aggregations, instances=instances)
...
def output_time_hist(data, threshold):
    ...
    for key, label in columns.iteritems():
        column = data[key]
        axes.bar(data.index, column['count'], label=label, width=10, align='center')

    axes.set_xlabel(data.index.name)
    axes.set_ylabel('instances < ' + str(threshold) + ' milliseconds')
    results.plot_axes(data=data, axes=axes, log_scale=False, y_min=0)

    results.output('time_hist', data=data, columns=columns, aggregations=aggregations, instances=None)
