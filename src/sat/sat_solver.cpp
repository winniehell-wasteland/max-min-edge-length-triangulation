                                 ...
                                 bool complete )
{
    ...
    if( complete )
    {
        ...
        for( SegmentIndex segment_index = problem.upper_bound();
...
void DecisionSATSolver::solve_decision_problem( const Settings& settings,
                                                const QString& file_prefix,
                                                const SATProblem& problem,
                                                SATSolution& solution )
{
    init_decision_problem( &problem );
    prepare_problem( problem, settings.complete_sat );

    if( settings.dump_sat_problems )
    {
...
void OptimizationSATSolver::solve_optimization_problem( const Settings& settings,
                                                        const QString& file_prefix,
                                                        const SATProblem& problem,
                                                        SATSolution& solution )
{
