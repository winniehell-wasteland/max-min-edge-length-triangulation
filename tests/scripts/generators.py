...
class SquarePointGenerator(PointGenerator):
	def __init__(self, coordinate_range):
		self._base = RectanglePointGenerator(x_range=coordinate_range, y_range=coordinate_range)
	...
	def __str__(self):
		return 'random points out of square {{{coordinate_range[0]},...,{coordinate_range[1]}}}^2' \
		.format(**self.parameters)

	def generate_point(self):
		return self._base.generate_point()

	@classmethod
	def parameter_formats(cls):
		return '{coordinate_range[0]:04}','{coordinate_range[1]:04}'

class CirclePointGenerator(PointGenerator):
	def __init__(self, radius, center=None):
		if not center:
			center = (radius, radius)

		self._base = RectanglePointGenerator(
	...
	@classmethod
	def parameter_formats(cls):
		return '{center[0]:04}','{center[1]:04}','{radius:04}'

	def generate_point(self):
		while True:
			point = self._base.generate_point()
